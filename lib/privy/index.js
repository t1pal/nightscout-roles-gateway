...
function configure (opts, server, persist) {
  ...
  function suggest_generic_inclusion_payload (req, res, next) {
    ...
    console.log("INCOMING EMAILS to INCLUDE", incoming);
    ...
    var includes = _.chain(candidates)
      ...
      .map(merge)
      ;
    console.log("ADJUSTING EMAILS", includes);
  ...
  function search_inclusions (req, res, next) {
    ...
    if (req.params.group_id) {
      query['site_acls.group_id'] = req.params.group_id;
    }
    ...
    sdk.toSession(undefined, req.header('Cookie'))
      ...
      .catch(function (error) {
        if (error.response && error.response.status == '401') {
          ...
          console.log("Kratos unavailable or error, falling back to anonymous:", error.code || error.message);
...
exports.adjustEmailSpec = adjustEmailSpec;
