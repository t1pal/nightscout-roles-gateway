...
const crypto = require('crypto');
  ...
  *
  ...
  status, synopsis,
  details: passing, warning, problem
  results
    criterium
      name
      property
...
function static_analysis (cfg) {
  ...
  var hasValidHostname = Boolean(link.hostname && link.hostname.trim());
  results.push({
    group: 'Nightscout URL',
    property: 'url syntax',
    criteria: 'should have a hostname',
    outcome: `hostname is ${link.hostname}`,
    passing: hasValidHostname,
    mandatory: true
  });
...
exports.static_analysis = static_analysis;
exports.check_api_secret_syntax = check_api_secret_syntax;
