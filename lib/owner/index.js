...
function configure (opts, server, persist) {
  ...
  function suggest_policy_payload (req, res, next) {
    ...
    var keepers = [ 'nickname', 'synopsis', 'notes', 'policy_name', 'policy_note', 'schedule_type', 'fill_pattern', 'schedule_segments' ];
  ...
  function create_suggested_policy (req, res, next) {
    ...
    var filter = _.pick(payload, ['owner_ref', 'expected_name', 'group_id', 'id' ]);
    SitePolicyDetails.db.(payload).then(function (err) {
      res.locals.inserted = {
        err
      , payload 
      };
