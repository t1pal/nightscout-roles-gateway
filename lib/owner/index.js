...
function configure (opts, server, persist) {
  ...
  function get_group_attributes (req, res, next) {
    var query = { 'owner_group_usage.owner_ref': req.params.owner_ref
    ...
    Group.db.findBy(query)
      .join('group_definitions', 'group_definitions.id', 'owner_group_usage.group_id')
    .paginate(pagination)
  ...
  function search_inclusions (req, res, next) {
    ...
    var props = _.pick(_.merge({ }, req.query, req.params), required);
    if (props.identity_type) {
      query.identity_type = props.identity_type;
    }
    if (props.identity_spec) {
      query.identity_spec = props.identity_spec;
    }
    res.locals.query = query;
    var page_props = ['perPage', 'currentPage', 'from', 'to'];
    var pagination = _.pick(_.merge({ perPage: 10 }, req.query), page_props);
  ...
  function suggest_group_payload (req, res, next) {
    console.log("PREV SUGGESTION", res.locals.suggestion);
    ...
    if (!_.isArray(incoming.includes) && _.isObject(incoming.includes)
  ...
  function update_user_group (req, res, next) {
    var fields = _.clone(res.locals.suggestion.group);
    if (_.isEmpty(fields) || req.query.dryrun) {
      res.locals.payload = { group: fields };
      return next( );
    }
    var filter = {
      owner_ref: req.params.owner_ref,
      id: req.params.group_id
    };
    GroupDefinitions.db.update(fields, filter).then(function (err) {
  ...
  function delete_owned_site (req, res, next) {
    if (_.isEmpty(res.locals.data) || _.isEmpty(res.locals.data.id)) {
    ...
    return persist.entities.Site.db.remove(query).then(function (removed) {
      res.status(204);
