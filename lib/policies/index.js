...
function lookup (env, server, persist) {
  ...
  function matches_api_secret (req, res, next) {
    ...
    var where = {
      expected_name: req.site.expected_name,
      is_enabled: true,
      exempt_matching_api_secret: true
    };
    var select = ['expected_name'];
    
    persist.entities.Site.db.findById(hashed_api_secret, select, where)
      .andWhere({ 'nightscout_secrets.expected_name': req.site.expected_name })
      .join('nightscout_secrets', 'nightscout_secrets.id', 'registered_sites.id')
