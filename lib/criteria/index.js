...
function configure (opts, server, persist) {
  ...
  */
  ...
  function fetch_audit_summaries (req, res, next) {
    var query = _.pick(_.merge({ }, req.query, req.params), [ 'owner_ref', 'expected_name', 'audit_id', 'upstream_origin', 'synopsis', 'status', 'acceptable' ]);
    var page_props = ['perPage', 'currentPage', 'from', 'to'];
    var pagination = _.pick(_.merge({ perPage: 4 }, req.query), page_props);
    AuditResults.db.findBy(query).orderBy('created_at', 'ASC').paginate(pagination).then(function (results) {
  ...
  function fetch_audit_details (req, res, next) {
    ...
    var page_props = ['perPage', 'currentPage', 'from', 'to'];
    var pagination = _.pick(_.merge({ perPage: 100 }, req.query), page_props);
