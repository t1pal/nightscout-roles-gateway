...
function mount (env, server) {
  ...
  var policies = Policies(env, server, entities);
  ...
  */
  ...
  server.get('/api/v1/workflows/site/registrations/:expected_name', registrations.handlers.suggest_registration, registrations.handlers.check_suggestion_available, locals_results( ));
  ...
  var saved_criteria = [ registrations.handlers.find_existing, criteria.handlers.existing_audit_config, criteria.handlers.triage, criteria.handlers.record_triaged_criteria ];
  ...
  server.post('/api/v1/workflows/site/registrations'
    , registrations.handlers.suggest_registration
    , registrations.handlers.find_existing
    , registrations.handlers.insert_new_site_registration
  ...
  server.post('/api/v1/workflows/site/registrations/:owner_ref/:expected_name/authenticity', registrations.handlers.suggest_registration, registrations.handlers.update_registration, saved_criteria, locals_results( ));
  ...
  */
  ...
  server.get('/api/v1/owner/:owner_ref/sites/:expected_name/inspections',  criteria.handlers.fetch_audit_summaries,  locals_results( ));
  server.get('/api/v1/owner/:owner_ref/sites/:expected_name/inspections/:audit_id',  criteria.handlers.fetch_audit_details, locals_results( ));

  // Delete Site
  server.del('/api/v1/owner/:owner_ref/sites/:expected_name'
    , synopsis.handlers.get_site_overview
    , clients.handlers.find_oath_clients
    , clients.handlers.remove_oauth_hydra_clients
    , registrations.handlers.remove_registration
    , locals_results( ));
  ...
  **/
  ...
  var tokens = require('./tokens/')(env, server, entities);
  server.get('/api/v1/owner/:owner_ref/sites/:expected_name/available/tokens'
    , registrations.handlers.find_existing
    , criteria.handlers.existing_audit_config
    , tokens.handlers.fetch_tokens
    , locals_results( ));
  /**
  ...
  **/
  ...
  var exchanged = require('./exchanged')(env, server, entities);
  ...
  server.get('/warden/v1/active/backend/for/:expected_name', policies.handlers.find_expected_name, privy.handlers.kratos_whoami, policies.handlers.get_acl_by_identity_param, exchanged.handlers.exchange_acl_token, policies( ), policies.handlers.deny_site_prefs, policies.handlers.matches_api_secret, policies.handlers.decision, exchanged.handlers.set_acl_token_header, policies.handlers.specify_upstream_handler, locals_results( ));
