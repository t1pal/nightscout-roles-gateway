...
function configure (opts, server, persist) {
  ...
  function check_suggestion_available (req, res, next) {
    ...
    var origin_query = DisallowedInfo.db.find([
      knex.raw('count(DISTINCT(upstream_origin))'),
      ]).whereRaw('? SIMILAR TO upstream_origin', [info.upstream_origin])
      .groupBy('upstream_origin')
    ;
    Promise.all([name_query, origin_query])
      .then(function (results) {

        var names = results[0];
        var origins = results[1];
        console.log("ALL RESPONES", arguments);
        var total = names.length + origins.length;
        res.locals.suggestion.available = total == 0;
