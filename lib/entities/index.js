...
function config (opts, server) {
  ...
  function set_entity (kind) {

    function set_entity (req, res, next) {
      req.entity = entities[kind];
      next( );
    }
    return set_entity;
  }
  ...
  function glean (cfg, attrs) {
    ...
    var disallowed = readonly_props.concat(cfg.readonly);
    if (cfg.strict) {
  ...
  function forge (cfg, attrs) {
    ...
    var disallowed = _.difference(readonly_props.concat(cfg.readonly), cfg.required);
    if (cfg.strict) {
  ...
  make.genid = genid;
  make.readonly_props = readonly_props;
  make.handlers = {
    elect_entity_table
  , elect_entity
