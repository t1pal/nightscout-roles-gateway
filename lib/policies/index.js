...
function lookup (env, server, persist) {
  ...
  function find_expected_name (req, res, next) {
    var query = { 'registered_sites.expected_name': req.params.expected_name };
    if (env.upstream.strictly_nightscout) { }
    var selects = [
      'registered_sites.*'
    , 'nightscout_authenticity_records.upstream_origin as confirmed_upstream'
    , 'nightscout_authenticity_records.status'
    , 'nightscout_authenticity_records.acceptable'
    ];
    persist.entities.Site.db
      .findBy(query, selects).leftJoin('nightscout_authenticity_records', 'nightscout_authenticity_records.expected_name', 'registered_sites.expected_name')
      .then(function (rows) {
      if (rows.length == 1) {
        req.site = rows[0];
        next( );
  ...
  function decision (req, res, next) {
    ...
    console.log("DECISION POINT", res.locals.policy, res.locals.acl);
  ...
  function matches_api_secret (req, res, next) {
    ...
    return persist.entities.Site.db.findById(hashed_api_secret, select, where).andWhere({ 'nightscout_secrets.expected_name': req.site.expected_name, 'registered_sites.is_enabled': true, 'registered_sites.exempt_matching_api_secret': true } ).join('nightscout_secrets', 'nightscout_secrets.id', 'registered_sites.id').then(function (matches) {
  ...
  function specify_upstream_handler (res, res, next) {
    if (res.locals.active) {
      var upstream = url.parse(res.locals.policy.site.upstream_origin);
      res.header('x-upstream-origin', res.locals.policy.site.upstream_origin);
      res.header('x-forwarded-host', upstream.hostname);
  ...
  function get_acl_by_identity_param (req, res, next) {
    ...
    var filters = {
      ...
      email: req.header('x-email-spec'),
      group: req.header('x-group-spec'),
      subject: req.params.subject || req.user.id,
